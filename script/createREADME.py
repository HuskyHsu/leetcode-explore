...
color = ["EF9A9A", "B39DDB", "81D4FA", "A5D6A7", "FFF59D", "FFAB91", "B0BEC5"]
backslash_char = "\n\n"
...
def create_readme(folder: str, info: dict) -> None:
    ...
    topic_tags = [declare_tag(i, tag) for i, tag in enumerate(info["topicTags"])]

    labels = [tag_label(tag) for i, tag in enumerate(info["topicTags"])]
